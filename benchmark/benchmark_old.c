...
void benchmark_memset_alignment_cache(const char* test_name, void* ptr, size_t len, int value, int iterations, bool flush_cache) {
    unsigned long long system_cycles = 0;
    unsigned long long ft_cycles = 0;
    ...
    const char* alignment = ((uintptr)ptr & 15) == 0 ? "Aligned  " : "Unaligned";
    const char* cache_state = flush_cache ? "Cold" : "Hot ";
    
    printf("%-20s | %s | %s | System: %6llu | ft_memset: %6llu | Ratio: %.2fx\n", 
           test_name, alignment, cache_state, system_cycles, ft_cycles, 
           (double)ft_cycles / system_cycles);
}

void benchmark_memset_detailed(const char* size_name, test_string* ts, int value) {
    printf("\n%s - Individual measurements (5 runs each):\n", size_name);
    ...
    for (int i = 0; i < 5; i++) {
        unsigned long long start = __rdtsc();
        memset(ts->aligned_str, value, ts->length);
        unsigned long long end = __rdtsc();
        printf("%6llu ", end - start);
    }
    printf("| ft_memset: ");
    for (int i = 0; i < 5; i++) {
        unsigned long long start = __rdtsc();
        ft_memset(ts->aligned_str, value, ts->length);
        unsigned long long end = __rdtsc();
        printf("%6llu ", end - start);
    }
    printf("\n");
    ...
    for (int i = 0; i < 5; i++) {
        unsigned long long start = __rdtsc();
        memset(ts->unaligned_str, value, ts->length);
        unsigned long long end = __rdtsc();
        printf("%6llu ", end - start);
    }
    printf("| ft_memset: ");
    for (int i = 0; i < 5; i++) {
        unsigned long long start = __rdtsc();
        ft_memset(ts->unaligned_str, value, ts->length);
        unsigned long long end = __rdtsc();
        printf("%6llu ", end - start);
    }
    printf("\n");
    ...
    printf("Aligned + Cold     - System: ");
    for (int i = 0; i < 5; i++) {
        flush_cache_lines(ts->aligned_str, ts->length);
        unsigned long long start = __rdtsc();
        memset(ts->aligned_str, value, ts->length);
        unsigned long long end = __rdtsc();
        printf("%6llu ", end - start);
...
void benchmark_memset() {
    ...
    printf("Memory Set Benchmark - Alignment & Cache Effects (averaged over %d iterations)\n", iterations);
    printf("==================================================================================\n");
    printf("%-20s | %-9s | %-4s | %-13s | %-13s | %s\n", 
           "Size", "Alignment", "Cache", "System (cyc)", "ft_memset (cyc)", "Ratio");
    printf("==================================================================================\n");
    
    // Test all combinations: aligned/unaligned × cached/uncached
    benchmark_memset_alignment_cache("Short (10 bytes)", short_ts.aligned_str, short_ts.length, test_value, iterations, false);
    benchmark_memset_alignment_cache("Short (10 bytes)", short_ts.unaligned_str, short_ts.length, test_value, iterations, false);
    benchmark_memset_alignment_cache("Short (10 bytes)", short_ts.aligned_str, short_ts.length, test_value, iterations, true);
    benchmark_memset_alignment_cache("Short (10 bytes)", short_ts.unaligned_str, short_ts.length, test_value, iterations, true);
    
    printf("----------------------------------------------------------------------------------\n");
    
    benchmark_memset_alignment_cache("Mid (1K bytes)", mid_ts.aligned_str, mid_ts.length, test_value, iterations, false);
    benchmark_memset_alignment_cache("Mid (1K bytes)", mid_ts.unaligned_str, mid_ts.length, test_value, iterations, false);
    benchmark_memset_alignment_cache("Mid (1K bytes)", mid_ts.aligned_str, mid_ts.length, test_value, iterations, true);
    benchmark_memset_alignment_cache("Mid (1K bytes)", mid_ts.unaligned_str, mid_ts.length, test_value, iterations, true);
    
    printf("----------------------------------------------------------------------------------\n");
    
    benchmark_memset_alignment_cache("Long (10K bytes)", long_ts.aligned_str, long_ts.length, test_value, iterations, false);
    benchmark_memset_alignment_cache("Long (10K bytes)", long_ts.unaligned_str, long_ts.length, test_value, iterations, false);
    benchmark_memset_alignment_cache("Long (10K bytes)", long_ts.aligned_str, long_ts.length, test_value, iterations, true);
    benchmark_memset_alignment_cache("Long (10K bytes)", long_ts.unaligned_str, long_ts.length, test_value, iterations, true);
    
    printf("----------------------------------------------------------------------------------\n");
    
    benchmark_memset_alignment_cache("Very Long (100K)", very_long_ts.aligned_str, very_long_ts.length, test_value, iterations, false);
    benchmark_memset_alignment_cache("Very Long (100K)", very_long_ts.unaligned_str, very_long_ts.length, test_value, iterations, false);
    benchmark_memset_alignment_cache("Very Long (100K)", very_long_ts.aligned_str, very_long_ts.length, test_value, iterations, true);
    benchmark_memset_alignment_cache("Very Long (100K)", very_long_ts.unaligned_str, very_long_ts.length, test_value, iterations, true);
    
    // Show detailed individual measurements for key sizes
    printf("\n\nDetailed Individual Measurements:\n");
    printf("====================================\n");
    benchmark_memset_detailed("Short (10 bytes)", &short_ts, test_value);
    benchmark_memset_detailed("Mid (1K bytes)", &mid_ts, test_value);
    benchmark_memset_detailed("Long (10K bytes)", &long_ts, test_value);
    
    // Memory alignment analysis
    printf("\n\nMemory Alignment Analysis:\n");
    printf("==========================\n");
    printf("Short buffers  - Aligned: %p (mod 16 = %lu), Unaligned: %p (mod 16 = %lu)\n", 
           (void*)short_ts.aligned_str, (uintptr)short_ts.aligned_str & 15,
           (void*)short_ts.unaligned_str, (uintptr)short_ts.unaligned_str & 15);
    printf("Mid buffers    - Aligned: %p (mod 16 = %lu), Unaligned: %p (mod 16 = %lu)\n", 
           (void*)mid_ts.aligned_str, (uintptr)mid_ts.aligned_str & 15,
           (void*)mid_ts.unaligned_str, (uintptr)mid_ts.unaligned_str & 15);
    printf("Long buffers   - Aligned: %p (mod 16 = %lu), Unaligned: %p (mod 16 = %lu)\n", 
...
int main() {
	srand(time(NULL));
	printf("=======================================================\n");
	printf("UNOPTIMIZED LIBFT BENCHMARKS\n");
	printf("=======================================================\n\n");
